...
use hsr_scanner::{
    ...
    reference::load_gilore_reference_bundle,
...
pub fn spawn_scan(settings: &StarRailSettings, status: Arc<Mutex<TaskStatus>>) -> TaskHandle {
    ...
    worker::spawn_cancellable_task(
        ...
        move |cancel| {
            ...
            let references = load_gilore_reference_bundle(&settings.reference_bundle).map_err(
                |error| {
                    hsr_ui_error(
                        UiText::new(
                            "无法加载星穹铁道参考数据。请选择由 GIlore 生成的完整参考数据文件夹。",
                            "Star Rail reference data could not be loaded. Choose a complete reference-data folder generated by GIlore.",
                        ),
                        error,
                    )
                },
            )?;
            references.validate_live_complete_profile().map_err(|error| {
                hsr_ui_error(
                    UiText::new(
                        "这份星穹铁道参考数据不完整，无法用于实机扫描。请更新后重试。",
                        "This Star Rail reference bundle is incomplete and cannot be used for a live scan. Update it, then retry.",
                    ),
                    error,
                )
            })?;
        ...
        move |cancel| {
            ...
            let references = load_gilore_reference_bundle(&settings.reference_bundle).map_err(
                |error| {
                    hsr_ui_error(
                        UiText::new(
                            "无法加载星穹铁道参考数据。请选择由 GIlore 生成的参考数据文件夹。",
                            "Star Rail reference data could not be loaded. Choose a reference-data folder generated by GIlore.",
                        ),
                        error,
                    )
                },
        ...
        move |cancel| {
            ...
            let references = load_gilore_reference_bundle(&settings.reference_bundle).map_err(
                |error| {
                    hsr_ui_error(
                        UiText::new(
                            "无法加载星穹铁道参考数据；不会执行任何游戏操作。",
                            "Star Rail reference data could not be loaded; no game action was performed.",
                        ),
                        error,
                    )
                },
            ...
            references.validate_live_complete_profile().map_err(|error| {
                hsr_ui_error(
                    UiText::new(
                        "星穹铁道参考数据不完整；不会执行任何游戏操作。",
                        "The Star Rail reference bundle is incomplete; no game action was performed.",
                    ),
                    error,
                )
            })?;
