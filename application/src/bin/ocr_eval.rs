...
fn main() -> Result<()> {
    ...
    let scan: Export =
        serde_json::from_str(&std::fs::read_to_string(scan_path).context("reading scan JSON")?)
            .context("parsing scan JSON")?;
    println!(
        "  artifacts={} characters={} weapons={}",
        scan.artifacts.len(),
        scan.characters.len(),
        scan.weapons.len()
    );
    ...
    let gt: Export =
        serde_json::from_str(&std::fs::read_to_string(gt_path).context("reading GT JSON")?)
    ...
    if art_dir.exists() && !scan.artifacts.is_empty() {
        ...
        for a in &gt.artifacts {
            gt_lookup.entry(artifact_match_key(a)).or_default().push(a);
        }
        let mut used: HashMap<String, Vec<bool>> = HashMap::new();
        for (k, v) in &gt_lookup {
        ...
        for (idx, scan_art) in scan.artifacts.iter().enumerate() {
            ...
            if let Some(gt_a) = gt_art {
                let gt_subs: Vec<&SubStat> = gt_a
                    .substats
                    .iter()
                    .chain(gt_a.unactivated_substats.iter())
                    .collect();

                for line in 0..4u32 {
                    let sub_path = dir.join(format!("sub{}.png", line));
                    if line as usize >= gt_subs.len() {
                        break;
                    }
                    ...
                    let v4_ok = p4.as_ref().map_or(false, |p| {
                        p.key == gt_sub.key && (p.value - gt_sub.value).abs() < 0.05
                    });
                    let v5_ok = p5.as_ref().map_or(false, |p| {
                        p.key == gt_sub.key && (p.value - gt_sub.value).abs() < 0.05
                    });
                ...
                art_equip.record(
                    ...
                    &t5,
                    true,
                    same, // v4 always "baseline", v5 correct if matches v4
                );
            }
        ...
        for (idx, scan_wpn) in scan.weapons.iter().enumerate() {
            ...
            let gt_wpn = if let Some(candidates) = gt_by_key.get(&mk) {
                ...
                for &(gi, gw) in candidates {
                    ...
                    let mut score = 0i32;
                    if gw.level == scan_wpn.level {
                        score += 10;
                    }
                    if gw.refinement == scan_wpn.refinement {
                        score += 5;
                    }
                    if gw.location == scan_wpn.location {
                        score += 5;
                    }
                    if best.is_none() || score > best.unwrap().0 {
                        best = Some((score, gi));
            ...
            wpn_level.record(
                &format!("[{:04}]", idx),
                &gt_wpn.level.to_string(),
                &format!("{:?}", n4),
                &format!("{:?}", n5),
                n4 == Some(gt_wpn.level),
                n5 == Some(gt_wpn.level),
            );
            ...
            wpn_refine.record(
                &format!("[{:04}]", idx),
                &gt_wpn.refinement.to_string(),
                &format!("{:?}", n4),
                &format!("{:?}", n5),
                n4 == Some(gt_wpn.refinement),
                n5 == Some(gt_wpn.refinement),
            );
    ...
    if char_dir.exists() && !scan.characters.is_empty() {
        ...
        let gt_char_map: HashMap<&str, &Character> =
            gt.characters.iter().map(|c| (c.key.as_str(), c)).collect();
        ...
        for (idx, scan_chr) in scan.characters.iter().enumerate() {
            ...
            chr_level.record(
                &format!("[{:04}]", idx),
                &gt_chr.level.to_string(),
                &format!("{:?}", n4),
                &format!("{:?}", n5),
                n4 == Some(gt_chr.level),
                n5 == Some(gt_chr.level),
            );
