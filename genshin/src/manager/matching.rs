...
mod tests {
    ...
    fn make_artifact_with_unactivated(
        ...
        art.unactivated_substats = unact
            .iter()
            .map(|(k, v)| GoodSubStat {
                key: k.to_string(),
                value: *v,
    ...
    fn test_hard_field_mismatch() {
        ...
        assert!(match_score(
            &scanned,
            &make_artifact("GladiatorsFinale", "plume", 5, 20, "hp", &[])
        )
        .is_none());
        assert!(match_score(
            &scanned,
            &make_artifact("GladiatorsFinale", "flower", 4, 20, "hp", &[])
        )
        .is_none());
        assert!(match_score(
            &scanned,
            &make_artifact("GladiatorsFinale", "flower", 5, 16, "hp", &[])
        )
        .is_none());
    }
    ...
    fn test_substat_key_mismatch_rejects() {
        ...
        assert!(
            match_score(&scanned, &target).is_none(),
            "Different substat keys must reject"
        );
    }
    ...
    fn test_substat_value_outside_tolerance_rejects() {
        ...
        assert!(
            match_score(&scanned, &target).is_none(),
            "Values outside 0.1 tolerance must reject"
        );
    }
    ...
    fn test_substat_value_within_tolerance_matches() {
        ...
        assert!(
            match_score(&scanned, &target).is_some(),
            "Values within 0.1 tolerance must match"
        );
    }
    ...
    fn test_substat_value_at_tolerance_boundary() {
        ...
        assert!(
            match_score(&scanned, &target).is_some(),
            "Exactly 0.1 diff must match"
        );

        // 0.11 diff should reject
        let scanned2 = make_artifact(
            "GladiatorsFinale",
            "flower",
            5,
            20,
        ...
        assert!(
            match_score(&scanned2, &target).is_none(),
            "0.11 diff must reject"
        );
    }
    ...
    fn test_substat_count_mismatch_rejects() {
        ...
        assert!(
            match_score(&scanned, &target).is_none(),
            "Different substat count must reject"
        );
    }
    ...
    fn test_unactivated_substats_hard_match() {
        ...
        assert!(
            match_score(&scanned, &target_wrong_key).is_none(),
            "Wrong unactivated key must reject"
        );
        assert!(
            match_score(&scanned, &target_no_unact).is_none(),
            "Missing unactivated must reject"
        );
    }

    #[test]
    fn test_elixir_crafted_hard_match() {
        let mut scanned = make_artifact(
            "GladiatorsFinale",
            "flower",
            5,
            20,
        ...
        let mut target_elixir = make_artifact(
            "GladiatorsFinale",
            "flower",
            5,
            20,
        ...
        let target_normal = make_artifact(
            "GladiatorsFinale",
            "flower",
            5,
            20,
        ...
        assert!(
            match_score(&scanned, &target_elixir).is_some(),
            "Same elixir status must match"
        );
        assert!(
            match_score(&scanned, &target_normal).is_none(),
            "Different elixir status must reject"
        );
    }
}
