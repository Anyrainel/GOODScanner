...
| Five-star artifact filter | `genshin/src/scanner/common/pixel_utils.rs` | SDR active filter brightness `>= 128`; HDR active filter brightness `>= 120` at `ARTIFACT_FIVE_STAR_FILTER_POS` | First 100 HDR per-item dumps sampled active as `RGB(117,129,145)`, brightness `130.3`, at scaled `(2496,280)` |
